    ...
    class Character
    {
        ...
        public void Attack(Skill skill, Character other)
        {
            switch (skill.type)
            {
                ...
                case SkillType.Heal:
                    {
                        int heal = CalcDamage(attack, skill.data1, other.def);
                        other.AddHp(heal);
                        Console.WriteLine($"{name}使用了{skill.name}技能，治疗{heal}点生命力~~");
                ...
                case SkillType.HealOverTime:
                    {
                        ...
                        Console.WriteLine($"{name}使用了治疗技能，治疗{heal}点生命力,并加上了{state.name}状态");
                        Console.WriteLine($"{name} 体力：{hp}/{maxHp}");
                    }
                    break;
            }
